locals {
  alarms_logs = {
    ...
    "content-cache-hydrator-error-logs" = {
      alarm_description = "Content Cache Hydrator: error in last run"
...
resource "aws_cloudwatch_log_metric_filter" "server_lambda_error_logs" {
  ...
  metric_transformation {
    name      = local.alarms_logs.server-lambda-error-logs.metric_name
...
resource "aws_cloudwatch_log_metric_filter" "content_cache_hydrator_error_logs" {
  ...
  metric_transformation {
    name      = local.alarms_logs.content-cache-hydrator-error-logs.metric_name
    namespace = var.prefix
    value     = "1"
    unit      = "Count"
  }
