...
test.describe("E2E", () => {
  ...
  test("Not eligible - age and catchup bullet points", async ({ browser }) => {
  ...
  test("Actionable - catchup bullet points", async ({ browser }) => {
  ...
  test("Actionable - InfoText action content", async ({ browser}) => {
    page = await login(browser, users.Actionable.email);
    await page.goto(RSV_PAGE_URL);
