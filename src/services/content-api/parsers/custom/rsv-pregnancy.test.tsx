import { styleHowToGetSubsection } from "@src/services/content-api/parsers/custom/rsv-pregnancy";
...
describe("styleHowToGetSubsection", () => {
  ...
  it("renders HTML if both regexes match", () => {
    ...
    const { container } = render(<>{styleHowToGetSubsection(subsection, 0)}</>);
    expect(container.innerHTML).toBe(
      '<div><div><p>Paragraph 1</p><p>Paragraph 2</p></div><p>In some areas you can also <a href="#" target="_blank" rel="noopener">book an RSV vaccination in a pharmacy</a>.</p></div>',
    );
  });
});
