...
describe("sanitiseHtml", () => {
  it("should add noopener to links with target already set", () => {
    ...
    const expectedHyperlink =
      '<a href="https://www.vita-test.uk/" target="_blank" rel="noopener">Link 1</a>';
  ...
  it("should set all links that do not have target to open externally", () => {
    const htmlLinkWithoutTarget =
      '<div><p>Text for <a href="https://www.vita-test.uk/">Link 1</a> content.</p></div>';
    const expectedHtmlWithTarget =
      '<a href="https://www.vita-test.uk/" target="_blank" rel="noopener">Link 1</a>';
  ...
  it("should not amend rel attribute on other non-link elements", () => {
    const htmlWithTargetAttributeOnNonAnchorElement =
      "<div><p>paragraph content.</p></div>";

    const sanitisedHtml = sanitiseHtml(
