...
type UpdatedToken = {
  [p: string]: unknown;
  user: { nhs_number: string; birthdate: string };
  expires_at: number;
...
const updateTokenWithValuesFromAccount = (token: JWT, account: Account, profile:Profile, nowInSeconds: number, maxAgeInSeconds: number) => {
  let jti = "";
...
const accessTokenIsMissingOrExpired = (updatedToken: UpdatedToken, nowInSeconds: number) => {
  return !updatedToken.expires_at || nowInSeconds >= updatedToken.expires_at;
}

const callRefreshTokenEndpointAndUpdateToken = async (config: AppConfig, updatedToken: UpdatedToken, nowInSeconds: number) => {
  ...
  if (!response.ok) {
    // todo: check the response body is an error before logging it?
    log.error(`callRefreshTokenEndpointAndUpdateToken: Error response status ${response.status} response body ${JSON.stringify(responseBody)}`);
    // TODO: how does the line below work; how can you throw a json blob? or is this an odd usage overload of the 'Error' object?
    throw responseBody;
  }

  // TODO: confirm what this line does if those fields are missing; does it throw?
  const newTokens = responseBody as {
    access_token: string;
    expires_in?: number;
    refresh_token?: string;
  };
...
const getToken = async (
  ...
  // TODO: refactor out the multiple return points, if possible
  if (!token) {
  ...
  try {
    ...
    if (isInitialLoginJourney(account, profile) && account != null && profile != null) {
      updatedToken = updateTokenWithValuesFromAccount(updatedToken, account, profile, nowInSeconds, maxAgeInSeconds)
      return updatedToken;
    }

    else {
      // Refresh token scenario: Access Token missing or expired
      if (accessTokenIsMissingOrExpired(updatedToken, nowInSeconds)) {
        ...
        if (!updatedToken.refresh_token) {
          log.error("getToken: unable to refresh token: refresh_token value is missing");
          return null;
        }
