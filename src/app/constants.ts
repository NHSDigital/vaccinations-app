export const VACCINATIONS_HUB_PAGE_ROUTE = "/check-and-book-rsv";
export const SERVICE_HEADING = "Check and book an RSV vaccination";
export const VACCINES_FOR_ALL_PAGE = "Vaccines for all ages";
export const NHS_TITLE_SUFFIX = "NHS";
export const HUB_FEEDBACK_REFERRER_ID = "hub";

export const HEADINGS = {
  WHAT_VACCINE_IS_FOR: "What the vaccine is for",
  WHO_SHOULD_HAVE_VACCINE: "Who should have the vaccine",
  HOW_TO_GET_VACCINE: "How to get the vaccine",
  VACCINE_SIDE_EFFECTS: "Side effects of the vaccine",
  IF_THIS_APPLIES: "If this applies to you",
};
